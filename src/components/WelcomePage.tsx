...
const WelcomePage: React.FC<WelcomePageProps> = ({ onStartQuiz }) => {
  ...
  const handleStartQuiz = () => {
    // Initialize the live leaderboard state
    ...
    <div className="min-h-[80vh] flex flex-col items-center justify-center px-4">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-bold mb-4">
          Welcome to the <span className="text-polkadot-pink">Polkadot</span>{" "}
          Quiz
        </h1>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Test your knowledge of Polkadot blockchain and compete with others on
          your local network!
        </p>
      </div>
      ...
      {serverInfo && isConnected && (
        <Alert className="max-w-md mb-6">
          ...
          <AlertDescription>
            Server: {serverInfo.ip}:{serverInfo.port} | Participants:{" "}
            {participants.length}
          </AlertDescription>
        </Alert>
      )}
        ...
        <CardContent>
          <ul className="space-y-2 mb-4">
            <li className="flex items-center gap-2">
              <span className="w-5 h-5 rounded-full bg-polkadot-pink text-white flex items-center justify-center text-xs">
                ✓
              </span>
